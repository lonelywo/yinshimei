...
public class CardMaker2 {
    ...
    public static List<CardBean> initCards(ShareHomeImgBean.DataBean share_info) {
        List<CardBean> list = new ArrayList<>();
        List<ShareHomeImgBean.DataBean.GoodsListBean> goods_list = share_info.getGoods_list();
        for (int i = 0; i <goods_list.size() ; i++) {
            CardBean cardBean = new CardBean();
            cardBean.setQrcode(goods_list.get(i).getQrcode());
            cardBean.setPoster(goods_list.get(i).getPoster());
